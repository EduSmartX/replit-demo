...
export function PasswordReset({ onBack, onSuccess }: PasswordResetProps) {
  ...
  const onStep1Submit = async (data: z.infer<typeof step1Schema>) => {
    ...
    try {
      ...
      if (err?.errors?.non_field_errors?.[0]) {
        errorMessage = err.errors.non_field_errors[0];
      } else if (err?.errors?.username?.[0]) {
        errorMessage = err.errors.username[0];
  ...
  const onStep2Submit = async (data: z.infer<typeof step2Schema>) => {
    ...
    try {
      ...
      if (err?.errors?.otp?.[0]) {
        form2.setError("otp", {
          type: "manual",
          message: err.errors.otp[0],
        });
        errorMessage = err.errors.otp[0];
      } else if (err?.errors?.new_password?.[0]) {
        form2.setError("newPassword", {
          type: "manual",
          message: err.errors.new_password[0],
        });
        errorMessage = err.errors.new_password[0];
      } else if (err?.errors?.confirm_password?.[0]) {
        form2.setError("confirmPassword", {
          type: "manual",
          message: err.errors.confirm_password[0],
        });
        errorMessage = err.errors.confirm_password[0];
      } else if (err?.errors?.non_field_errors?.[0]) {
        errorMessage = err.errors.non_field_errors[0];
      } else if (err?.non_field_errors?.[0]) {
        errorMessage = err.non_field_errors[0];
      } else if (err?.detail && typeof err.detail === "string") {
