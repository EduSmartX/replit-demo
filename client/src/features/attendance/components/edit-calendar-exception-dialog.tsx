/**
 * Edit Calendar Exception Dialog
 * Dialog for editing an existing calendar exception
 */
...
import { updateCalendarException } from "@/lib/api/calendar-exception-api";
import type { CalendarException, CalendarExceptionUpdate } from "@/lib/api/calendar-exception-types";
...
interface EditCalendarExceptionDialogProps {
...
export function EditCalendarExceptionDialog({
  exception,
  open,
  onOpenChange,
  onSuccess,
}: EditCalendarExceptionDialogProps) {
  ...
  // Update selected classes when exception changes
  useEffect(() => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: CalendarExceptionUpdate) =>
      updateCalendarException(exception.public_id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["calendar-exceptions"] });
      toast({
        title: "Success",
        description: "Calendar exception updated successfully",
      });
      onSuccess?.();
    },
  ...
  const onSubmit = (values: FormValues) => {
    ...
    // Validation
    if (!payload.is_applicable_to_all_classes && (!payload.classes || payload.classes.length === 0)) {
      toast({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the exceptional work policy details
          </DialogDescription>
        </DialogHeader>
