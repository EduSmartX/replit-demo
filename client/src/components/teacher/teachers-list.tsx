...
import { Plus, Pencil, Trash2, Eye, Users, Filter, X } from "lucide-react";
...
export function TeachersList({ onCreateNew, onView, onEdit, onDelete }: TeachersListProps) {
  ...
  // Fetch teachers with filters (fetch all without pagination for client-side filtering)
  const {
    data: teachersData,
    isLoading,
    error,
    ...
    queryKey: ["teachers", filterSpecialization, filterDesignation, searchQuery],
    queryFn: () =>
      fetchTeachers({
        page: 1,
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mt-10 mb-8 flex items-center justify-between">
        <div>
          <h1 className="mb-3 text-3xl font-bold text-gray-900">Teachers Management</h1>
          <p className="text-base text-gray-600">Manage and track teachers in your organization</p>
        </div>
